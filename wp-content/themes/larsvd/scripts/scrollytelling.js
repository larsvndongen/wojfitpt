jQuery(document).ready(function ($) {
  ...
  var numSlides = jQuery('.row.flex-content.r36 .gsap-container .block.repeater .slide').length;
  var slideAmount = numSlides * 100;
  

  // Get the progress bar element
  var progressBar = jQuery('.row.flex-content.r36 .block.progress .custom-full-row');
  ...
  jQuery('.row.flex-content.r36 .block.progress article').each(function (index) {
    var left = index === 0 ? 0 : index * leftPercentage; // Set 0% for the first ball, calculate position for the rest
    jQuery(this).css('left', left + '%');
  });

  // gsap.to(".row.flex-content.r36 .progress-group", {
  //   scrollTrigger: {
