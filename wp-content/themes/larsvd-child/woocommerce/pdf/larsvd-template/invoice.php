	...
	<tr>
		...
		<td class="shop-info">
			...
			<div class="company-info">
				<p>KvK-nummer: 94710198</p>
				<p>Btw-nummer: NL005103808B54</p>
			</div>
		</td>
	</tr>
</table>
				...
				<td class="product">
					...
					<div class="item-meta">
						<?php if (!empty($item['sku'])): ?>
							<p class="sku"><span class="label"><?php $this->sku_title(); ?></span>
								<?php echo esc_attr($item['sku']); ?></p>
						<?php endif; ?>
						<?php if (!empty($item['weight'])): ?>
							<p class="weight"><span class="label"><?php $this->weight_title(); ?></span>
				...
				<table class="totals">
					<tfoot>
						...
						foreach ($this->get_woocommerce_totals() as $key => $total) {
							if ($key === 'cart_subtotal') { // Zorg ervoor dat je het subtotaal (exclusief btw) gebruikt
								$totalPrice = $total['value'];
							}
						}

						// Maak van de waarde een getal
						$totalPrice = preg_replace('/[^0-9,]/', '', $totalPrice);
						$totalPrice = str_replace(',', '.', $totalPrice);
						$totalPrice = floatval($totalPrice);

						// Haal btw-percentage op vanuit WooCommerce (optioneel)
						$taxRate = 0.21; // Standaard 21%
						if (function_exists('WC_Tax') && class_exists('WC_Tax')) {
							$taxRates = WC_Tax::get_rates(); // Ophalen van belastingtarieven
							if (!empty($taxRates)) {
								$rate = reset($taxRates);
								$taxRate = $rate['rate'] / 100; // Conversie naar decimaal
							}
						}
						...
						<?php if ($taxAmount): ?>
							<tr>
								<th>Belasting (<?= ($taxRate * 100) ?>%)</th>
								<td>€<?= number_format($taxAmount, 2, ',', '.') ?></td>
							</tr>
						<?php endif; ?>

						<?php if ($totalPriceIncludingTax): ?>
							<tr>
								<th>Betaald</th>
								<td>€<?= number_format($totalPriceIncludingTax, 2, ',', '.') ?></td>
							</tr>
						<?php endif; ?>
					</tfoot>
				</table>
